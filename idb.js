(function () {
    ...
    // Паддинги оранжевого бокса = паддинги блока оценки IMDb
    ...
    function showRating(rating, votes) {
        ...
        if (votes !== 'N/A') {
            // Берём вычисленные стили именно с блока оценки IMDb
            var imdbStyle = window.getComputedStyle(imdbBlock[0]);

            var votesBox = $('<div></div>')
                .addClass('full-start__rate imdb-votes-box') // сохраняем базовую геометрию
                .text(votes)
                .css({
                    'background': '#c2410c', // наш оранжевый
                    'color': '#fff',
                    'font-weight': 'bold',
                    'margin-right': '0.5em',
                    ...
                    // Копируем скругление и другие нужные стили
