(function () {
    ...
    // Плагин: Чистый IMDb рейтинг + блок с полным числом голосов (тёмно-оранжевый, как возрастной)
    ...
    function extractData(json) {
        ...
        var votesFormatted = votesRaw;

        if (votesRaw !== 'N/A') {
            // Показываем полное число с запятыми (как в OMDb)
            votesFormatted = votesRaw; // уже приходит как "654,005"
        }
    ...
    function showRating(rating, votes) {
        ...
        // Блок голосов — точная копия возрастного, но тёмно-оранжевый фон + полное число
        var ageBlock = $('.full-start__pg', render);
        if (ageBlock.length && votes !== 'N/A') {
            ageBlock.before(
                '<div class="imdb-votes-box" style="' +
                'background: #c2410c !important;' +           // тёмно-оранжевый
                'color: #fff !important;' +
