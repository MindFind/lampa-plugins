(function () {
    'use strict';
    // Плагин: IMDb рейтинг + количество голосов (полное число)
    // Оранжевый бокс имеет ТОЧНО такие же паддинги, как блок оценки IMDb
    // Ключ OMDb: 479575b3
    ...
    function showRating(rating, votes) {
        ...
        // Оранжевый бокс с голосами
        if (votes !== 'N/A') {
            // Копируем вычисленные стили паддингов именно с блока оценки IMDb
            var imdbStyle = window.getComputedStyle(imdbBlock[0]);

            var votesBox = $('<div></div>')
                .addClass('full-start__rate imdb-votes-box') // базовая геометрия
                .text(votes)
                .css({
                    'background': '#c2410c', // тёмно-оранжевый
                    'color': '#fff',
                    'font-weight': 'bold',
                    'margin-right': '0.5em',

                    // ТОЧНО копируем паддинги с блока оценки IMDb
                    ...
                    // Копируем скругление и другие свойства
